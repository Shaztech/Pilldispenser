  ...
  for (int i = 1; i <= 10; i++) {
    if (trayAlertEna[i] && !alreadyTriggered[i] && trayEnabled[i] && !traydisptoday[i] && !traydismtoday[i]) {
      if (trayHours[i] == timeinfo.tm_hour && trayMin[i] == timeinfo.tm_min) {
        ...
        if (DISCORD_WEBHOOK != "" && !discord_alertonce) {
          int discordinterval;
          discord_alertonce = true;
          switch (discordalertinterval) {
            case 0: discordinterval = 100; break; // Disabled
            case 1: discordinterval = 30000; break; // Instant (30sec)
            case 2: discordinterval = 5 * 60 * 1000; break; // 5 min
            case 3: discordinterval = 10 * 60 * 1000; break; // 10 min
            case 4: discordinterval = 15 * 60 * 1000; break; // 15 min
            case 5: discordinterval = 30 * 60 * 1000; break; // 30 min
            case 6: discordinterval = 45 * 60 * 1000; break; // 45 min
            case 7: discordinterval = 60 * 60 * 1000; break; // 60 min
          }
          lv_timer_create(discord_timer, discordinterval, NULL);
          lv_timer_create(discord_resetalert, 5000, NULL);
        }
...
static void discord_resetalert(lv_timer_t * timer) {
  discord_alertonce = false;
  lv_timer_del(timer);
}
...
void sendDiscordMessage(const String& message) {
  ...
  client.setInsecure(); // Skip cert verification for Discord

  // Parse host and path from webhook URL
  // Format: https://discord.com/api/webhooks/ID/TOKEN
  String url = DISCORD_WEBHOOK;
  url.replace("https://", "");
...
static void discord_timer(lv_timer_t * timer) {
  if (alertinprogress && discordalertinterval > 0 && internetstatus != 0) {
