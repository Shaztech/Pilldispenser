...
static void update_time_label(lv_timer_t * timer) {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    char scrTimeString[6]; // Screensaver: "12:30\0" (no AM/PM, font is digits-only)
    if (clock12hr) {
      strftime(timeString, sizeof(timeString), "%I:%M %p", &timeinfo);
      strftime(scrTimeString, sizeof(scrTimeString), "%I:%M", &timeinfo);
      // Show AM/PM on separate label (montserrat font has letters)
      char ampm[3];
      strftime(ampm, sizeof(ampm), "%p", &timeinfo);
      lv_label_set_text(ui_ClockSCRAMPM, ampm);
      lv_obj_clear_flag(ui_ClockSCRAMPM, LV_OBJ_FLAG_HIDDEN);
    } else {
      strftime(timeString, sizeof(timeString), "%H:%M", &timeinfo);
      strftime(scrTimeString, sizeof(scrTimeString), "%H:%M", &timeinfo);
      lv_obj_add_flag(ui_ClockSCRAMPM, LV_OBJ_FLAG_HIDDEN);
    }
    lv_label_set_text(ui_ClockLBL, timeString);
    lv_label_set_text(ui_ClockSCRLBL, scrTimeString);
